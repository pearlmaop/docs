...
This guide helps you get started with the Neo4j [vector store](/oss/integrations/vectorstores). For detailed documentation of all `Neo4jVectorStore` features and configurations, see the [API reference](https://reference.langchain.com/javascript/langchain-neo4j/Neo4jVectorStore).
...
Install `@langchain/neo4j`, the MongoDB-compatible `neo4j-driver`, `@langchain/core`, and an embeddings provider (this guide uses [OpenAI embeddings](/oss/integrations/embeddings/openai)):
