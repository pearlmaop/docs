...
This guide helps you get started with the turbopuffer [vector store](/oss/integrations/vectorstores). For detailed documentation of all `TurbopufferVectorStore` features and configurations, see the [API reference](https://reference.langchain.com/javascript/langchain-turbopuffer/TurbopufferVectorStore).
...
[Sign up for a turbopuffer account](https://turbopuffer.com/join), create an API key, and install `@langchain/turbopuffer`, the official `@turbopuffer/turbopuffer` client, `@langchain/core`, and an embeddings provider (this guide uses [OpenAI embeddings](/oss/integrations/embeddings/openai)).
