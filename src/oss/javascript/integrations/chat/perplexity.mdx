...
const chat = new ChatPerplexity({ model: "openai/gpt-5.4" });
...
See the [Perplexity Agent API model list](https://docs.perplexity.ai/docs/agent-api/models) for the full set of models available through this endpoint (e.g. `openai/gpt-5.4`, `anthropic/claude-sonnet-4-6`, `google/gemini-3-1-pro`).

## Related integrations

The `@langchain/perplexity` package also includes search components that do not use the chat API:

- [`PerplexitySearchRetriever`](/oss/integrations/retrievers/perplexity_search): returns `Document` objects from the Perplexity Search API for RAG pipelines
- [`PerplexitySearchResults`](/oss/integrations/tools/perplexity_search): agent tool that returns JSON search results

See the [Perplexity provider overview](/oss/integrations/providers/perplexity) for setup on all three components.
