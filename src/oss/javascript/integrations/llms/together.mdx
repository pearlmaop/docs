...
To access the legacy `TogetherAI` completions class, create a Together account, [get an API key](https://api.together.xyz/), and install the `@langchain/together-ai` integration package. For chat and instruct models, prefer [`ChatTogetherAI`](/oss/integrations/chat/togetherai).
